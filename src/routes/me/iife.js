(() => {
  ((doc, hasInitialized) => {
      ...
      if (void 0 === containerEl._qwikjson_) {
        ...
        while (script) {
          if (
            ...
            "qwik/json" === getAttribute(script, "type")
          ) {
    ...
    const dispatch = async (element, onPrefix, ev, eventName = ev.type) => {
      ...
      const qrls =
        null == ctx ? void 0 : ctx.li.filter((li) => li[0] === attrName);
      if (qrls && qrls.length > 0) {
        for (const q of qrls) {
          await q[1].getFn([element, ev], () => element.isConnected)(
            ev,
            element
          );
        }
    ...
    const processDocumentEvent = async (ev) => {
      ...
      while (element && element.getAttribute) {
        ...
        element =
          ev.bubbles && !0 !== ev.cancelBubble ? element.parentElement : null;
      }
    };
